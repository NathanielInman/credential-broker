...
export async function userModify(name) {
  try {
    ...
    for (let i = 0; i < updatedUser.permissions.scopes.length; i++) {
      const scopeName = updatedUser.permissions.scopes[i].name
      ...
      do {
        ...
        if (!bool) {
          answer = await prompt(
            chalk.green(`Allow full access (Y) or view access to scope "${scopeName}"?`)
          )
          bool = true
        } // end if
      } while (!bool)

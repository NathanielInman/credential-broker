...
import { authSecure, authSecureEncrypt, authSecureDecrypt } from '../libraries/authSecure.js'

export async function userInitialize() {
  let user
  ...
  if (fs.existsSync('./user.json')) {
    ...
    if (
      await confirm(
        chalk.green('User (') + chalk.magenta(user.name) + chalk.green(') exists already, remove?')
      )
    ) {
  ...
  spinner.start()
  await sleep(1000)
  fs.writeFileSync('./id_rsa.pub', key.publicKey)
  await sleep(1000)
  readline.cursorTo(process.stdout, 0)
  ...
  spinner.start()
  await sleep(1000)
  fs.writeFileSync('./id_rsa', key.privateKey)
  await sleep(1000)
  readline.cursorTo(process.stdout, 0)
  ...
  user.permissions.viewScopeNames = await confirm(
    chalk.green('Request view all scopes permission? ')
  )
  ...
  do {
    ...
    for (let i = 0; i < scopeNames.length; i++) {
      answer = await confirm(
        chalk.green(`Request full access (Y) or view access to scope "${scopeNames[i]}"(n)?`)
      )
    ...
    if (!scopeNames.length) {
      ...
      answer = await confirm(
        chalk.green(
          `Is this correct: "${user.permissions.scopes

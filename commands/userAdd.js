...
export async function userAdd(name) {
  ...
  newUser.permissions.viewScopeNames = await confirm(
    chalk.green('Allow view all scopes permission? ')
  )
  ...
  do {
    ...
    for (let i = 0; i < scopeNames.length; i++) {
      answer = await confirm(
        chalk.green(`Allow full access (Y) or view access to scope "${scopeNames[i]}"(n)?`)
      )
      newUser.permissions.scopes.push({ name: scopeNames[i], value: answer ? 'edit' : 'view' })
    } // end for
    answer = await confirm(
      chalk.green(
        `Is this correct: "${newUser.permissions.scopes
